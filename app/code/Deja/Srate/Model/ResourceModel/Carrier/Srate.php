 ...
 * CedCommerce
  ...
  */
namespace Deja\Srate\Model\ResourceModel\Carrier;

use Magento\Framework\Filesystem;
use Magento\Framework\Filesystem\DirectoryList;


class Srate extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
 ...
 //   protected $_importUniqueHash = [];
    ...
    public function __construct(
        ...
        \Deja\Srate\Model\Carrier\Srate $carrierTablerate,
    ...
    public function getRates(\Magento\Quote\Model\Quote\Address\RateRequest $request)
    {
        ...
        $condition = $this->_coreConfig->getValue('carriers/srate/ratecondition', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        ...
        $methods = array();
        $rates = array();
        $weight_type = $this->_coreConfig->getValue('carriers/srate/weight_type', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        ...
        $dimensional_condition = $this->_coreConfig->getValue('carriers/srate/dimensional_calculation', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                ...
                if (!in_array($value['shipping_method'] , $methods)) {
                    ...
                    if ($dimensional_condition==1) {
                        foreach ($items as $item) {
                            ...
                            $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/srate.log');
                            $logger = new \Zend\Log\Logger();
                            $logger->addWriter($writer);
    ...
    public function uploadAndImport(\Magento\Framework\DataObject $object)
    {
        ...
        if (empty($_FILES['groups']['tmp_name']['srate']['fields']['import']['value'])) {
            return $this;
        }
        $csvFile = $_FILES['groups']['tmp_name']['srate']['fields']['import']['value'];
 ...
 //       $this->_importUniqueHash = [];
        ...
        if ($headers === false || count($headers) < 12) {
            $stream->close();
            throw new \Magento\Framework\Exception\LocalizedException(__('Please correct Rates File Format.'));
        }
 ...
 */
     ...
     * @param array $data
     * @return \Deja\Srate\Model\Resource\Carrier\Srate
     */
